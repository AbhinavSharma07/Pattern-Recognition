...
class AntiPatternVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        # Process custom rules for function calls
        for rule in self.custom_rules:
            if rule.get("enabled") and rule.get("type") == "Call":
                # unparse() converts the AST node back to a string like 'os.system'
                call_name = ast.unparse(node.func)
                if call_name == rule.get("match"):
                    self._record_smell(node, rule.get("name", "Custom rule match"))

        # Ensure we continue visiting other nodes
        self.generic_visit(node)
